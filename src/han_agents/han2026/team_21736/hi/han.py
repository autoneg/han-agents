...
class Han(SAONegotiator):
    ...
    Phase 0 – Exploration  (t < 0.15) : offer near-best, gather data
    Phase 1 – Boulware     (t < 0.75) : slow concession curve
    Phase 2 – Accelerate   (t < 0.90) : faster concessions, ACNext active
    Phase 3 – Deadline     (t < 1.00) : accept above reservation, send urgency
    """

    # ------------------------------------------------------------------
    # Message banks as CLASS attributes
    # (never referenced as bare module-level names → no NameError)
    # ------------------------------------------------------------------
    _OPENING = [
        "Thank you for joining this negotiation. "
        "I look forward to reaching a mutually beneficial agreement.",
    ...
    _HARD_MSG = [
        "This is an important issue for me; I cannot move much further on this point.",
    ...
    def on_preferences_changed(self, changes=None):
        """Invalidate cached outcomes when preferences are updated."""
        self._sorted_outcomes = None
        self._first_turn = True

    # ------------------------------------------------------- cached outcomes
    def _get_sorted_outcomes(self) -> list:
        if self._sorted_outcomes is not None:
    ...
    def _aspiration(self, t: float) -> float:
        """
        Boulware curve: aspiration(t) = rv + (1-rv)*(1 - t^e)
    ...
    def _estimate_opponent_best_offer(self):
        """Frequency-based: return the opponent offer best for us."""
    ...
    def _outcome_at_utility(self, target_util: float):
        """Return outcome with utility closest to target_util from above."""
    ...
    def _build_counter_offer(self, t: float):
        """Combine aspiration target + opponent model to build best counter."""
    ...
    def _generate_text(self, t: float, response_type: str, offer) -> str:
        """Generate a persuasive Markdown message for the human partner."""
        ...
        if t >= self.deadline_start:
            ...
            if offer is not None:
                ...
                msg += f"\n\n*This offer gives me **{u:.0%}** of my maximum utility.*"
            return msg
    ...
    def __call__(self, state: SAOState, **kwargs) -> SAOResponse:
        """
        Main negotiation callback, called once per round by NegMAS.

        **kwargs absorbs any extra keyword arguments the server passes
        (e.g. dest=) without raising TypeError.
        """
        if self.ufun is None or self.nmi is None:
        ...
        if offer is not None:
            ...
            # (a) Meets current aspiration level
            if u_offer >= asp:
                return SAOResponse(
            ...
            # (b) ACNext: offer ≥ what we'd counter with minus small delta
            ...
            # (c) Deadline pressure: accept anything above reservation+margin
...
# ---------------------------------------------------------------------------
# Alias: keep HanNegotiator name working for local tests
# ---------------------------------------------------------------------------
HanNegotiator = Han
